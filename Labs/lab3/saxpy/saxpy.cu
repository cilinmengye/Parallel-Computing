...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    //
    /*
     * cudaMempy() is synchronous under the conditions we are using it. 
     */

     cudaMemcpy(device_x, xarray, size, cudaMemcpyHostToDevice);
     cudaMemcpy(device_y, yarray, size, cudaMemcpyHostToDevice);
    /*
     * After your additions, your program should time two executions:
     * You should also insert timers the measure only the time taken to run the kernel. 
     * (They should not include the time of CPU-to-GPU data transfer 
     * or transfer of results from the GPU back to the CPU.)
     */    
    // start timing after allocation of device memory
    double startTime = CycleTimer::currentSeconds();  

    /*
     * By defult a CUDA kernel's execution on the GPU is asynchronous with 
     * the main application thread running on the CPU. 
     */
