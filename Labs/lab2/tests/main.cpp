...
int main(int argc, char** argv)
{
    ...
    for (int test_id = 0; test_id < n_tests; test_id++) {
        ...
        // N_TASKSYS_IMPLS的含义各个版本的数量，总共4个
        // 版本包括[Serial]，[Parallel + Always Spawn]，[Parallel + Thread Pool + Spin] ，[Parallel + Thread Pool + Sleep]
        for (int i = 0; i < N_TASKSYS_IMPLS; i++) {
            double minT = 1e30;
            // num_timing_iterations为程序运行的迭代次数
            for (int j = 0; j < num_timing_iterations; j++) {

                // Create a new task system
                // selectTaskSystemRefImpl返回的ITaskSystem是我们需要在tasksys.cpp中实现的各个相关类
                // 即TaskSystemSerial，TaskSystemParallelSpawn，TaskSystemParallelThreadPoolSpinning，TaskSystemParallelThreadPoolSleeping
                ITaskSystem *t = selectTaskSystemRefImpl(num_threads, (TaskSystemType) i);

                // Run test
                // 其中的ITaskSystem *t貌似是一种安排，其中调用继承IRunnable类的类才是决定如何执行任务的
